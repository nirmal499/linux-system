...
int main(int argc, char const *argv[])
{
    ...
    /* Simple check for octal numbers and correct length */
    if (strspn(argv[1], "01234567\n") != strlen(argv[1]) || (strlen(argv[1]) != 3 &&
                                                             strlen(argv[1]) != 4))
    {
        printUsage(stderr, argv[0]);
        return 1;
    ...
    /* The chmod() system call takes two arguments, the path to the file or directory and the
        file permission expressed as an octal number 
        e.g 4755 octal is 2541
